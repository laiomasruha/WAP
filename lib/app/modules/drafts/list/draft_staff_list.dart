...
class DraftStaffList extends StatelessWidget {
  final TextEditingController _controller = new TextEditingController();
  final DraftsController controller;

  DraftStaffList({@required this.controller});

  @override
  Widget build(BuildContext context) {
    void _addItem() {
      if (_controller.text.trim().isNotEmpty) {
        controller.add(_controller.text.trim().toUpperCase(), ListType.staff);
        _controller.clear();
      }
    }
    ...
    void _reorderItem(bool plus) {
      ...
      if (0 <= controller.indexOrdering + factor &&
          controller.indexOrdering + factor < controller.draft.staff.length) {
        String staff = controller.draft.staff[controller.indexOrdering];
        controller.remove(controller.indexOrdering, ListType.staff);
        controller.indexOrdering = controller.indexOrdering + factor;
        controller.insert(controller.indexOrdering, staff, ListType.staff);
      }
    }
    ...
    return Scaffold(
      appBar: AppBar(
        title: Observer(
          builder: (_) => controller.isOrdering == true
              ? SizedBox()
              : Text(
                  functions.ucWords(lists.arrayList[ListType.staff.index]),
      ...
      body: Column(
        children: <Widget>[
          Expanded(
            child: Observer(
              builder: (_) => (controller.draft.staff != null &&
                      controller.draft.staff.length > 0)
                  ? ListView.builder(
                      shrinkWrap: true,
                      itemCount: controller.draft.staff != null
                          ? controller.draft.staff.length
                          : 1,
                      itemBuilder: (context, index) {
                        ...
                        _controller1.text =
                            controller.get(index, ListType.staff);
                            ...
                            IconSlideAction(
                              ...
                              onTap: () async {
                                final item = await Modular.link.pushNamed(
                                    "/edit/${controller.draft.staff[index]}");

                                if (item != null) {
                                  controller.update(
                                      index, item, ListType.staff);
                                }
                              },
                            ),
                            IconSlideAction(
                              ...
                              onTap: () {
                                ...
                                controller.remove(index, ListType.staff);
