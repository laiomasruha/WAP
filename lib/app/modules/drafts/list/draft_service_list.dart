...
class DraftServiceList extends StatelessWidget {
  final TextEditingController _controller = new TextEditingController();
  final DraftsController controller;

  DraftServiceList({@required this.controller});

  @override
  Widget build(BuildContext context) {
    void _addItem() {
      if (_controller.text.trim().isNotEmpty) {
        controller.add(_controller.text.trim().toUpperCase(), ListType.services);
        _controller.clear();
      }
    }
    ...
    void _reorderItem(bool plus) {
      ...
      if (0 <= controller.indexOrdering + factor &&
          controller.indexOrdering + factor <
              controller.draft.services.length) {
        String service = controller.draft.services[controller.indexOrdering];
        controller.remove(controller.indexOrdering, ListType.services);
        controller.indexOrdering = controller.indexOrdering + factor;
        controller.insert(controller.indexOrdering, service, ListType.services);
      }
    }
    ...
    return Scaffold(
      appBar: AppBar(
        title: Observer(
          builder: (_) => controller.isOrdering == true
              ? SizedBox()
              : Text(
                  functions.ucWords(lists.arrayList[ListType.services.index]),
      ...
      body: Column(
        children: <Widget>[
          Expanded(
            child: Observer(
              builder: (_) => (controller.draft.services != null &&
                      controller.draft.services.length > 0)
                  ? ListView.builder(
                      shrinkWrap: true,
                      itemCount: controller.draft.services != null
                          ? controller.draft.services.length
                          : 1,
                      itemBuilder: (context, index) {
                        ...
                        _controller1.text =
                            controller.get(index, ListType.services);
                        ...
                        return Slidable(
                          ...
                          child: Container(
                            child: ListTile(
                              title: Text(controller.draft.services[index]),
                              leading: Observer(
                                builder: (_) => controller.isOrdering == true
                                    ? Icon(
                                        Icons.swap_vert,
                            ...
                            IconSlideAction(
                              ...
                              onTap: () async {
                                final item = await Modular.link.pushNamed(
                                    "/edit/${controller.draft.services[index]}");

                                if (item != null) {
                                  controller.update(
                                      index, item, ListType.services);
                                }
                              },
                            ),
                            IconSlideAction(
                              ...
                              onTap: () {
                                ...
                                controller.remove(index, ListType.services);
print(_lastIndexRemoved);
print(_lastServiceRemoved);
